...
def get_band_resonance_center_frequency(band):
    """
    Calculates the center resonance frequency of an amateur radio band.
    ...
    if band in rac_band_plan:
        ...
        try:
            ...
            upper_freq_str = frequency_range.split('-')[1].strip().split(' ')[0]  # Handle units
            lower_freq = float(lower_freq_str)
            upper_freq = float(upper_freq_str)
            center_frequency = lower_freq + (upper_freq - lower_freq) / 2
    ...
    while True:
        ...
        if choice == 'A':
            ...
            if frequency:
                ...
                print(f"\nInvalid band choice. \n Wrong Number or Did you forget to add m (eg: 20m)? \n Please check provided band list or the RAC website.\n")

        elif choice == 'B':
            # Option B: Calculate length correction from measured offset
            print("\nCalculate length correction from a measured offset for these Available bands: ")
            for band in rac_band_plan.keys():
                print(band, end=", ")
            print()  # newline
            print()  # 2nd Newline
